...
const processPackSets = ({ account, pubkey }, setter) => {
    ...
    try {
        if (isPackSetAccount(account)) {
            const packSet = PackSet_1.decodePackSet(account.data);
            const parsedAccount = {
                pubkey,
                account: account,
                info: packSet,
...
const isPackAccount = (account) => account && utils_1.pubkeyToString(account.owner) === utils_1.PACK_CREATE_ID.toString();
