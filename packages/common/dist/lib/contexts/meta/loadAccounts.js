...
const pullPage = async (connection, page, tempCache) => {
    ...
    if (account) {
        ...
        if (store) {
            await PackSet_1.getPackSets({ connection, storeId: store }).then(forEach(processPackSets_1.processPackSets));
        }
        if (page == 0) {
...
const loadAccounts = async (connection) => {
    ...
    const STORE_ID = __1.programIds().store;
    const forEach = (fn) => async (accounts) => {
        for (const account of accounts) {
    ...
    const loadPacks = () => PackSet_1.getPackSets({ connection, storeId: STORE_ID }).then(forEachAccount(processPackSets_1.processPackSets));
    const loading = [
